...
# TechInt Web - Sistema de Gestión 🖥️  
[![Spring Boot](https://img.shields.io/badge/Spring%20Boot-3.1.5-green)](https://spring.io/projects/spring-boot)  
[![MySQL](https://img.shields.io/badge/MySQL-8.0-blue)](https://www.mysql.com/)  

Aplicación web basada en roles desarrollada con Spring Boot para gestionar empresas, usuarios y contratos, creada como parte de **Programación II** en el INSPT-UTN.  
...
- **Control de acceso por roles**:  
  - **Administradores**: Crear usuarios, países, áreas operativas (ej. Marketing, Finanzas) y empresas.  
  - **Vendedores**: Crear sub-vendedores y registrar ventas.  
  - **Asesores**: Establecer contratos de consultoría con empresas.  
- **Soporte multi-entorno**: Cambiar entre bases de datos local y de producción usando Spring Profiles.  
- **Integración con Thymeleaf**: Plantillas HTML dinámicas para renderizado en el servidor.  
...
- **Backend**:  
  - Spring Boot MVC  
  - Spring Security (Acceso basado en roles)  
  - Spring Data JPA  
- **Frontend**:  
  - Thymeleaf  
- **Base de datos**:  
...
2. **Configurar la base de datos**:  
   ...
   - Crear una base de datos llamada `holdingDataBase`.  
...
### Perfiles de Spring  
- **Desarrollo local (perfil `dev`)**:  
  Usa la base de datos MySQL local. Activar con:  
  ```properties  
  spring.profiles.active=dev  
  ```  

- **Producción**:  
  Configurar AWS RDS y definir variables de entorno:  
  ```properties  
  spring.datasource.url=${DATASOURCE_URL}  
  spring.datasource.username=${DATASOURCE_USERNAME}  
  spring.datasource.password=${DATASOURCE_PASSWORD}  
...
- Modificar `httpd.conf` para:  
  - Vincular a `0.0.0.0:80`.  
  - Permitir acceso al puerto `3306` (MySQL).  

---

## 🔒 Seguridad  
- **Spring Security**: Restringe acceso según roles (`ADMIN`, `VENDOR`, `ADVISOR`).  
- **Encriptación de contraseñas**: Uso de BCrypt.  
- **Gestión de sesiones**: Autenticación basada en sesiones HTTP.  

---

## 🚨 Solución de Problemas  
- **Problemas de conexión a MySQL**:  
  - Verificar `bind-address = 0.0.0.0` en `my.ini`.  
  - Revisar reglas del firewall para los puertos `80` y `3306`.  
- **Activación de perfiles**:  
  Asegurar que el perfil correcto esté definido en `application.properties`.  
...
Let me know if you need adjustments to deployment URLs or additional technical details! 😊
